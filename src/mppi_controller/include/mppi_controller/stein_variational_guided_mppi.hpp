...
#include <functional> // if needed for std::function
...
namespace cpu {
    ...
    private:
        ...
        const double Vx_cov_;
        const double Vy_cov_;
        const double w_cov_;
        ...
        const double w_cov_for_grad_estimation_;
    ...
    private:
        ...
        // 1D Gaussian fitting for each dimension (if you want adapt in each dimension)
        std::pair<double, double> gaussian_fitting(const std::vector<double>& x,
                                                const std::vector<double>& y) const;

        // 4-argument dimension-wise fitting
        double dimension_gaussian_fitting(const std::vector<double>& ctrl_vals,
                                        const std::vector<double>& weights,
                                        double min_cov, double max_cov) const;
        // // If we want dimension-wise fitting
        // double dimension_gaussian_fitting(const std::vector<ControlSeq>& control_seq_history,
        //                                 const std::vector<double>& softmax_costs,
        //                                 const size_t dim_idx,
        //                                 const size_t step_idx) const;
    };
