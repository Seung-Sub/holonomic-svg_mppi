...
namespace cpu {
    ForwardMPPI::ForwardMPPI(const Params::Common& common_params, const Params::ForwardMPPI& forward_mppi_params)
        : prediction_step_size_(static_cast<size_t>(common_params.prediction_step_size)),
          ...
          Vx_cov_(forward_mppi_params.Vx_cov),
          Vy_cov_(forward_mppi_params.Vy_cov),
          w_cov_(forward_mppi_params.w_cov),
          ...
          Vx_cov_for_grad_estimation_(forward_mppi_params.Vx_cov_for_grad_estimation),
          Vy_cov_for_grad_estimation_(forward_mppi_params.Vy_cov_for_grad_estimation),
          w_cov_for_grad_estimation_(forward_mppi_params.w_cov_for_grad_estimation) {
        // ROS_INFO_THROTTLE(1.0, "ForwardMPPI initialization started");
        ...
        std::array<double, CONTROL_SPACE::dim> max_control_inputs = {common_params.max_Vx, common_params.max_Vy, common_params.max_w};
        std::array<double, CONTROL_SPACE::dim> min_control_inputs = {common_params.min_Vx, common_params.min_Vy, common_params.min_w};
        ...
        // Log gradient sampler initialization completion
        ROS_INFO_THROTTLE(1.0, "Gradient sampler initialization completed: sample_batch_num=%zu", sample_batch_num);
    ...
    void ForwardMPPI::set_reference_path(const nav_msgs::Path& reference_path) { mpc_base_ptr_->set_reference_path(reference_path); };
    ...
    ControlSeq ForwardMPPI::grad_reverse_kld(const ControlSeq& mean_seq,
        ...
        const ControlSeqCovMatrices grad_cov = sampler->get_constant_control_seq_cov_matrices({Vx_cov_for_grad_estimation_, Vy_cov_for_grad_estimation_, w_cov_for_grad_estimation_});
